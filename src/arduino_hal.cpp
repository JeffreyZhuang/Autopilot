...
Arduino_HAL::Arduino_HAL(Plane * plane): HAL(plane), spi_bus(PB5, PB4, PA5), imu(spi_bus, PC15), baro(PC14, &spi_bus, 20000000, OSR_ULTRA_HIGH), 
                                         i2c_bus(PB9, PB8), ina219(&i2c_bus, 0x40, 0.01), swo(2000000) {
    _plane = plane;
}

void Arduino_HAL::delay_us(uint32_t us) {
    delayMicroseconds(us);
}

void Arduino_HAL::swo_print(char * str) {
    swo.print(str);
}

void Arduino_HAL::usb_print(char * str) {
    Serial.print(str);
}

uint32_t Arduino_HAL::get_time_us() {
    return micros();
}

void Arduino_HAL::blink_led() {
...
void Arduino_HAL::setup_peripherals() {
    Serial.begin(115200);
    pinMode(PC1, OUTPUT);
...
void Arduino_HAL::setup_sensors() {
    imu.begin();
    baro.begin();
    mag.begin_SPI(PC13, &spi_bus);
}

void Arduino_HAL::setup_sd() {
